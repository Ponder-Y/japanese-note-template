...
function SettingButton({ icon, title, subtitle, on, onClick }) {
  return (
    <button className="sidebar-setting" type="button" aria-pressed={on} onClick={onClick}>
      ...
      <span className="sidebar-setting-copy"><strong>{title}</strong><small>{subtitle}</small></span>
      <ToggleSwitch on={on} />
    </button>
  );
}
...
function Heading({ id, title, level = 2 }) {
  ...
  const handleAnchor = async event => {
    ...
    try { await navigator.clipboard.writeText(window.location.href); } catch (_) { /* optional */ }
  };
...
function PracticeBlock({ block }) {
  ...
  return (
    <div className="practice-card">
      ...
      {open && <div className="answer-panel"><div className="answer-title">解答</div><div className="answer-text">{block.answer}</div><p>{block.explanation}</p></div>}
    </div>
  );
}
...
function Block({ block }) {
  switch (block.type) {
    ...
    case 'callout':
      return <aside className={`callout ${block.tone || 'info'}`}><div className="callout-icon">{block.tone === 'warning' ? '!' : 'i'}</div><div><strong>{block.title || ''}</strong><div dangerouslySetInnerHTML={{ __html: block.html }} /></div></aside>;
    case 'table':
      return <div className="table-wrap"><table><thead><tr>{block.headers.map(header => <th key={header}>{header}</th>)}</tr></thead><tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={`${rowIndex}-${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div>;
    case 'example':
    ...
    case 'dialogue':
      return <div className="dialogue">{block.lines.map((line, index) => <div className="dialogue-line" key={`${line.speaker}-${index}`}><div className="speaker">{line.speaker}</div><div><div className="dialogue-jp" dangerouslySetInnerHTML={{ __html: line.jp }} /><div className="dialogue-zh">{line.zh}</div></div></div>)}</div>;
    case 'practice':
...
export default function NotebookApp() {
  ...
  useEffect(() => localStorage.setItem('note-sidebar-collapsed', String(sidebarCollapsed)), [sidebarCollapsed]);
  ...
  useEffect(() => {
    ...
    const onChange = event => { if (event.matches) setMobileOpen(false); };
  ...
  useEffect(() => setActiveResult(index => Math.min(index, Math.max(0, searchItems.length - 1))), [searchItems.length]);
  ...
  const onSearchKeyDown = event => {
    ...
    } else if (event.key === 'Enter' && searchItems[activeResult]) {
      event.preventDefault();
    ...
    <>
      ...
      <dialog className="search-dialog" ref={searchDialogRef} aria-label="搜尋筆記" onClose={() => setSearchOpen(false)} onClick={event => { if (event.target === searchDialogRef.current) setSearchOpen(false); }}>
        <div className="search-panel">
          <div className="search-input-wrap"><Icon name="search" /><input ref={searchInputRef} type="search" value={searchQuery} onChange={event => setSearchQuery(event.target.value)} onKeyDown={onSearchKeyDown} placeholder="搜尋標題、內容或關鍵字…" autoComplete="off" /><kbd>Esc</kbd></div>
          <div className="search-results" role="listbox">
            {searchItems.length ? searchItems.map((item, index) => <button className={`search-result ${index === activeResult ? 'active' : ''}`} role="option" aria-selected={index === activeResult} type="button" onMouseEnter={() => setActiveResult(index)} onClick={() => navigate(item.slug)} key={item.slug}><span className="result-category">{item.note.category}</span><strong>{item.note.title}</strong><span>{item.note.description}</span></button>) : <div className="search-empty">找不到符合「{searchQuery}」的筆記</div>}
          </div>
